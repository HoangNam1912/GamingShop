          ...
          <div class="tile-body">
            <div class="row element-button">
              <div class="col-sm-2">
                ...
                <a class="btn btn-add btn-sm" href="views/admin/addsanpham.php" title="Thêm"><i class="fas fa-plus"></i>
                  Tạo mới sản phẩm</a>
              </div>
              ...
              <tbody class="product-list">
                <?php foreach ($products as $product) { ?>
                  <tr class="product">
                    ...
                    <td><img src="../images/hinh/<?php echo $product['image']; ?>" alt="" width="100px;"></td>
                    <td><?php echo $product['description']; ?></td>
                    <td><?php echo $product['price']; ?></td>
                    <td>
                    <button class="btn btn-primary btn-sm trash" type="button" title="Xóa" onclick="deleteProduct(<?php echo $product['product_id']; ?>)"><i class="fas fa-trash-alt"></i></button>
<button class="btn btn-primary btn-sm edit" type="button" title="Sửa" onclick="redirectToEdit(<?php echo $product['product_id']; ?>)">
...
function deleteProduct(productId) {
    if (confirm("Bạn có chắc muốn xóa sản phẩm này?")) {
        // Tạo một XMLHttpRequest để gửi yêu cầu xóa sản phẩm
        var xhr = new XMLHttpRequest();

        // Thiết lập phương thức và đường dẫn của yêu cầu
        xhr.open('POST', 'views/admin/deleteproduct.php', true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                document.getElementById("successMessage").innerText = "Đã xóa sản phẩm thành công!";
            } else {
                console.error("Xóa sản phẩm không thành công.");
            }
        };

        // Gửi yêu cầu với dữ liệu là ID của sản phẩm cần xóa
        xhr.send('productId=' + productId);
    }
}
</script>
<script>
    // Hàm để chuyển hướng và truyền ID sản phẩm khi click vào nút sửa
function redirectToEdit(productId) {
    // Chuyển hướng đến trang editproduct với ID sản phẩm được truyền qua URL
    window.location.href = 'views/admin/editproduct.php?productId=' + productId;
}
